...
// Listado
$sql = "
SELECT rpc.*, p.nombre AS promo_nombre, p.origen, p.fecha_inicio, p.fecha_fin
FROM recargas_promo_clientes rpc
...
<div class="container mt-3">
  ...
  <!-- <div class="alert alert-info py-2">
    ...
    <?php if (!RECARGAS_ENFORCE_WAIT): ?>
      Bloqueo por días <span class="badge bg-secondary">DESACTIVADO</span>.
    <?php else: ?>
      Bloqueo activo: R1 <?= (int)REC1_WAIT_DAYS ?> días, R2 <?= (int)REC2_WAIT_DAYS ?> días.
    <?php endif; ?>
  </div> -->
  ...
  <form class="row g-2 mb-2">
    ...
    <?php if ($promoId > 0 && in_array($ROL, ['Admin','Logistica'], true)): ?>
      <div class="col-md-3 d-flex align-items-end justify-content-end">
        <a
          class="btn btn-success"
          target="_blank" rel="noopener"
          href="export_recargas_csv.php?promo=<?= (int)$promoId ?>&status=all&q=<?= urlencode($q) ?>"
          title="Exporta todas las filas visibles para esta promo (según permisos)"
        >
          Exportar CSV
        </a>
      </div>
      ...
      <div class="col-12">
        <div class="text-muted small">
          ...
          <?php if (!in_array($ROL, ['Admin','Logistica'], true)): ?>
            — <strong>Sucursal:</strong> #<?= (int)$ID_SUCURSAL ?>
          <?php endif; ?>
        </div>
      </div>
  ...
  <div class="table-responsive">
    <table class="table table-sm align-middle">
      <thead>
        <tr>
          <th>Cliente</th><th>Teléfono</th><th>Venta</th><th>R1</th><th>R2</th><th>Acciones</th>
        </tr>
      </thead>
      <tbody>
    ...
    <form class="modal-content" method="post" action="recargas_confirmar.php" enctype="multipart/form-data" onsubmit="return validateUpload();">
      ...
      <div class="modal-body">
        ...
        <!-- <p class="text-muted small mb-0">
          <?php if (!RECARGAS_ENFORCE_WAIT): ?>
            Modo pruebas: sin bloqueo por días.
          <?php else: ?>
            Se validará que hayan transcurrido <?= (int)REC1_WAIT_DAYS ?> o <?= (int)REC2_WAIT_DAYS ?> días según aplique.
          <?php endif; ?>
        </p> -->
      </div>
