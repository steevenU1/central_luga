              ...
              foreach ($cols as $c) {
                ...
                switch ($c) {
                  ...
                  case 'comentario_aprobador':
                    $vals[] = 'Aprobado desde solicitud #' . $solId . ($obs ? (' · ' . $obs) : '');
                    $types .= 's';
                    break;
                  default:
                    if ($c === $dateCol) {
...
<body>
  <div class="container my-4">
    ...
    <div class="row g-3 mb-3">
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-slate"><i class="bi bi-people"></i>
          <div>
            <div class="text-muted small">Colaboradores</div>
            <div class="big"><?= (int)$empleadosActivos ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-green"><i class="bi bi-person-check"></i>
          <div>
            <div class="text-muted small">Presentes</div>
            <div class="big"><?= (int)($totAsis + $totRet) ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-yellow"><i class="bi bi-alarm"></i>
          <div>
            <div class="text-muted small">Retardos</div>
            <div class="big"><?= (int)$totRet ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-red"><i class="bi bi-person-x"></i>
          <div>
            <div class="text-muted small">Faltas</div>
            <div class="big"><?= (int)$totFal ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-purple"><i class="bi bi-clipboard-check"></i>
          <div>
            <div class="text-muted small">Permisos</div>
            <div class="big"><?= (int)$totPerm ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-slate"><i class="bi bi-moon-stars"></i>
          <div>
            <div class="text-muted small">Descansos</div>
            <div class="big"><?= (int)$totDesc ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-blue"><i class="bi bi-clock-history"></i>
          <div>
            <div class="text-muted small">Horas</div>
            <div class="big"><?= number_format($horasTot, 2) ?></div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-2">
        <div class="kpi bg-soft-green"><i class="bi bi-graph-up"></i>
          <div>
            <div class="text-muted small">Puntualidad</div>
            <div class="big"><?= $puntualidad ?>%</div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-3">
        <div class="kpi bg-soft-blue"><i class="bi bi-bullseye"></i>
          <div>
            <div class="text-muted small">Cumplimiento</div>
            <div class="big"><?= $cumplimiento ?>%</div>
          </div>
        </div>
      </div>
      <div class="col-6 col-md-3 col-xl-3">
        <div class="kpi bg-soft-yellow"><i class="bi bi-exclamation-diamond"></i>
          <div>
            <div class="text-muted small">Falta por 3+ retardos</div>
            <div class="big"><?= (int)$faltasPorRetardos ?></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="d-flex flex-wrap gap-2 mb-3">
      <a class="btn btn-outline-success btn-sm" href="?export=matrix&<?= $qsExport ?>">
        <i class="bi bi-grid-3x3-gap me-1"></i> Exportar matriz
      </a>

      <a class="btn btn-outline-primary btn-sm" href="?export=detalles&<?= $qsExport ?>">
        <i class="bi bi-file-earmark-spreadsheet me-1"></i> Exportar detalles
      </a>

      <a class="btn btn-outline-secondary btn-sm" href="?export=permisos&<?= $qsExport ?>">
        <i class="bi bi-clipboard-check me-1"></i> Exportar permisos
      </a>

      <button class="btn btn-danger btn-sm ms-auto" data-bs-toggle="modal" data-bs-target="#modalIncidencia">
        <i class="bi bi-exclamation-octagon me-1"></i> Agregar incidencia
      </button>

      <a class="btn btn-warning btn-sm" href="vacaciones_panel.php">
        <i class="bi bi-sunglasses me-1"></i> Ir al panel de vacaciones
      </a>
    </div>
        ...
        <div class="table-responsive">
          <table class="table table-striped table-xs align-middle mb-0">
            ...
            <tbody>
              <?php if (!$matriz): ?>
                <tr>
                  <td colspan="<?= 2 + count($days) + 8 ?>" class="text-muted">Sin datos.</td>
                </tr>
                    ...
                    <?php foreach ($fila['dias'] as $d):
                      ...
                      switch ($estado) {
                        case 'ASISTIÓ':
                          ...
                          break;
                        case 'RETARDO':
                          $abbr = 'R' . ($d['retardo_min'] > 0 ? '+' . $d['retardo_min'] . 'm' : '');
                          $cls = 'pill-R';
                          $title = 'Retardo';
                          break;
                        case 'FALTA':
        ...
        <div class="tab-content" id="asistTabsContent">
            ...
            <div class="table-responsive">
              <table class="table table-hover table-xs align-middle mb-0">
                ...
                <tbody>
                  <?php if (!$asistDetView): ?>
                    <tr>
                      <td colspan="10" class="text-muted">Sin registros.</td>
                    </tr>
              ...
              <table class="table table-striped table-xs align-middle mb-0">
                ...
                <tbody>
                  <?php if (!$permisosSemana): ?>
                    <tr>
                      <td colspan="8" class="text-muted">Sin permisos en esta semana.</td>
                    </tr>
          ...
          <div class="tab-pane fade" id="tab-sol-vac" role="tabpanel">
            <div class="d-flex align-items-center justify-content-between mb-2">
              <div class="fw-bold">Solicitudes pendientes</div>
              <div class="small text-muted">Aprobar = registra días en permisos_solicitudes como “Vacaciones”.</div>
            </div>
            ...
            <div class="table-responsive">
              <table class="table table-striped table-xs align-middle mb-0">
                ...
                <tbody>
                  <?php if (!$solVacPend): ?>
                    <tr>
                      <td colspan="7" class="text-muted">No hay solicitudes pendientes.</td>
                    </tr>
                      ...
                      <tr>
                        ...
                        <td class="text-end">
                          <form method="post" class="d-inline">
                            <input type="hidden" name="accion" value="resolver_solicitud_vacaciones">
                            <input type="hidden" name="sol_id" value="<?= (int)$sv['id'] ?>">
                          ...
                          <button type="button" class="btn btn-sm btn-outline-danger"
                            ...
                            data-bs-target="#modalRechazarVac"
                            data-sol-id="<?= (int)$sv['id'] ?>"
                            data-user="<?= h($sv['usuario']) ?>"
                            data-rango="<?= h(($sv['fecha_inicio'] ?? '') . ' → ' . ($sv['fecha_fin'] ?? '')) ?>">
  ...
  <!-- MODAL: Rechazar solicitud vacaciones -->
  <div class="modal fade" id="modalRechazarVac" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form method="post">
          <input type="hidden" name="accion" value="resolver_solicitud_vacaciones">
          <input type="hidden" name="decision" value="RECHAZAR">
          <input type="hidden" name="sol_id" id="rej-sol-id" value="0">
          ...
          <div class="modal-body">
            <div class="mb-2 small text-muted" id="rej-info"></div>
            <label class="form-label">Motivo / comentario (opcional)</label>
            <textarea class="form-control" name="comentario" rows="2" placeholder="Ej: No hay cobertura en esas fechas…"></textarea>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      const modal = document.getElementById('modalRechazarVac');
      if (!modal) return;
      ...
      modal.addEventListener('show.bs.modal', (ev) => {
        ...
        const solId = btn.getAttribute('data-sol-id') || '0';
        const user = btn.getAttribute('data-user') || '';
        const rango = btn.getAttribute('data-rango') || '';

        const inp = document.getElementById('rej-sol-id');
        const info = document.getElementById('rej-info');

        if (inp) inp.value = solId;
        if (info) info.textContent = `${user} · ${rango}`;
      });
    });
  </script>
