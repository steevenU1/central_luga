<?php
// promos_equipos_descuento_admin.php — FULL FIX + soporte promo precio fijo por financiera

if (session_status() === PHP_SESSION_NONE) { session_start(); }
if (!isset($_SESSION['id_usuario'])) { header("Location: index.php"); exit(); }

require_once __DIR__ . '/db.php'; // DB primero
...
function tipoPrecioHuman(array $p): string {
  $tipo = trim((string)($p['tipo_precio_combo'] ?? 'porcentaje'));
...
$hasPermiteCombo   = column_exists($conn, 'promos_equipos_descuento', 'permite_combo');
$hasPermiteDV      = column_exists($conn, 'promos_equipos_descuento', 'permite_doble_venta');
$hasFinanciera     = column_exists($conn, 'promos_equipos_descuento', 'financiera');
$hasTipoPrecio     = column_exists($conn, 'promos_equipos_descuento', 'tipo_precio_combo');
$hasPrecioFijo     = column_exists($conn, 'promos_equipos_descuento', 'precio_combo_fijo');
$hasMensajeModal   = column_exists($conn, 'promos_equipos_descuento', 'mensaje_modal');
...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_promo') {
      ...
      $tipo_precio      = trim((string)($_POST['tipo_precio_combo'] ?? 'porcentaje'));
      $precio_combo_fijo = (float)($_POST['precio_combo_fijo'] ?? 0);
      $mensaje_modal    = trim((string)($_POST['mensaje_modal'] ?? ''));

      if ($nombre === '') throw new Exception("El nombre es obligatorio.");

      if ($hasTipoPrecio) {
        if (!in_array($tipo_precio, ['porcentaje','precio_fijo'], true)) {
      ...
      if ($tipo_precio === 'porcentaje') {
        ...
        $precio_combo_fijo = 0.0;
      } else {
        if ($precio_combo_fijo <= 0) throw new Exception("El precio fijo del combo debe ser mayor a 0.");
        $pct = 0.0;
      }
      ...
      if ($tipo_precio === 'precio_fijo' && $hasPermiteCombo && $permite_combo !== 1) {
        throw new Exception("La promo de precio fijo debe permitir modo COMBO.");
      }
      ...
      $types = "sdiss";
      $params = [$nombre, $pct, $activa, $fi2, $ff2];

      if ($hasPermiteCombo) {
        $sets[] = "permite_combo=?";
        $types .= "i";
        $params[] = $permite_combo;
      }
      if ($hasPermiteDV) {
        $sets[] = "permite_doble_venta=?";
        $types .= "i";
        $params[] = $permite_dobleventa;
      }
      if ($hasFinanciera) {
        $sets[] = "financiera=?";
        $types .= "s";
      ...
      if ($hasTipoPrecio) {
        $sets[] = "tipo_precio_combo=?";
        $types .= "s";
        $params[] = $tipo_precio;
      }
      if ($hasPrecioFijo) {
        $sets[] = "precio_combo_fijo=?";
        $types .= "d";
        $params[] = $precio_combo_fijo;
      }
      if ($hasMensajeModal) {
        $sets[] = "mensaje_modal=?";
        $types .= "s";
        $params[] = $mensaje_modal;
      }

      if ($id > 0) {
        $sql = "UPDATE promos_equipos_descuento SET " . implode(", ", $sets) . " WHERE id=?";
        $types2 = $types . "i";
        $params[] = $id;
        $st = $conn->prepare($sql);
        $bind = [$types2];
        ...
        $cols = ["nombre","porcentaje_descuento","activa","fecha_inicio","fecha_fin"];
        $vals = ["?","?","?","NULLIF(?, '')","NULLIF(?, '')"];

        if ($hasPermiteCombo) { $cols[] = "permite_combo"; $vals[] = "?"; }
        if ($hasPermiteDV)    { $cols[] = "permite_doble_venta"; $vals[] = "?"; }
        if ($hasFinanciera)   { $cols[] = "financiera"; $vals[] = "?"; }
        if ($hasTipoPrecio)   { $cols[] = "tipo_precio_combo"; $vals[] = "?"; }
        if ($hasPrecioFijo)   { $cols[] = "precio_combo_fijo"; $vals[] = "?"; }
        if ($hasMensajeModal) { $cols[] = "mensaje_modal"; $vals[] = "?"; }

        $sql = "INSERT INTO promos_equipos_descuento (" . implode(",", $cols) . ") VALUES (" . implode(",", $vals) . ")";
        $st = $conn->prepare($sql);
        $bind = [$types];
        foreach ($params as $k => $v) $bind[] = &$params[$k];
        call_user_func_array([$st, 'bind_param'], $bind);
...
<div class="container my-4">
  ...
  <div class="d-flex align-items-center justify-content-between mb-3">
    <div>
      ...
      <div class="mini">Ahora soporta porcentaje y precio fijo por financiera.</div>
    </div>
      ...
      <div class="card card-elev">
        <div class="card-body">
          ...
          <form method="post">
            ...
            <?php if ($hasFinanciera): ?>
            <div class="mb-2">
              <label class="form-label">Financiera</label>
              <select class="form-select" name="financiera">
                <?php $finSel = (string)($promoEdit['financiera'] ?? ''); ?>
                <option value="" <?= $finSel === '' ? 'selected' : '' ?>>Todas / sin restricción</option>
                <option value="PayJoy" <?= $finSel === 'PayJoy' ? 'selected' : '' ?>>PayJoy</option>
                <option value="Krediya" <?= $finSel === 'Krediya' ? 'selected' : '' ?>>Krediya</option>
                <option value="Innovación Movil" <?= $finSel === 'Innovación Movil' ? 'selected' : '' ?>>Innovación Movil</option>
                <option value="Plata Card" <?= $finSel === 'Plata Card' ? 'selected' : '' ?>>Plata Card</option>
                <option value="LesPago" <?= $finSel === 'LesPago' ? 'selected' : '' ?>>LesPago</option>
              </select>
            </div>
            <?php endif; ?>

            <?php $tipoSel = (string)($promoEdit['tipo_precio_combo'] ?? 'porcentaje'); ?>
            <?php if ($hasTipoPrecio): ?>
            <div class="mb-2">
              <label class="form-label">Tipo de precio del combo</label>
              <select class="form-select" name="tipo_precio_combo" id="tipo_precio_combo">
                <option value="porcentaje" <?= $tipoSel === 'porcentaje' ? 'selected' : '' ?>>Porcentaje</option>
                <option value="precio_fijo" <?= $tipoSel === 'precio_fijo' ? 'selected' : '' ?>>Precio fijo</option>
              </select>
            </div>
            <?php endif; ?>

            <div class="mb-2" id="wrap_pct">
            ...
            <?php if ($hasPrecioFijo): ?>
            <div class="mb-2" id="wrap_precio_fijo">
              <label class="form-label">Precio fijo del combo</label>
              <input class="form-control" name="precio_combo_fijo" type="number" step="0.01" min="0"
                     value="<?= h($promoEdit['precio_combo_fijo'] ?? '0.00') ?>">
            </div>
            <?php endif; ?>

            <?php if ($hasMensajeModal): ?>
            <div class="mb-2">
              <label class="form-label">Mensaje modal</label>
              <textarea class="form-control" name="mensaje_modal" rows="3"><?= h($promoEdit['mensaje_modal'] ?? '') ?></textarea>
              <div class="mini mt-1">Opcional. Si lo dejas vacío, el sistema arma un mensaje automático.</div>
            </div>
            <?php endif; ?>
            ...
            <table class="table table-sm align-middle">
              <thead class="table-light">
                <tr>
                  ...
                  <th>Tipo</th>
                  <th>Valor</th>
                  <?php if ($hasFinanciera): ?><th>Financiera</th><?php endif; ?>
              ...
              <?php foreach ($promos as $p): ?>
                <?php
                  $tipoPrecio = (string)($p['tipo_precio_combo'] ?? 'porcentaje');
                  $valorHuman = ($tipoPrecio === 'precio_fijo')
                    ? '$' . number_format((float)($p['precio_combo_fijo'] ?? 0), 2)
                    : number_format((float)($p['porcentaje_descuento'] ?? 0), 2) . '%';
                ?>
                <tr>
                  <td class="mono"><?= (int)$p['id'] ?></td>
                  <td><?= h($p['nombre']) ?></td>
                  <td><?= h(tipoPrecioHuman($p)) ?></td>
                  <td><?= h($valorHuman) ?></td>
                  <?php if ($hasFinanciera): ?><td><?= h((string)($p['financiera'] ?? 'Todas')) ?></td><?php endif; ?>
                  <?php if ($hasPermiteCombo && $hasPermiteDV): ?>
              ...
              <?php if (count($promos) === 0): ?>
                <tr><td colspan="10" class="text-center mini">Aún no hay promos.</td></tr>
              <?php endif; ?>
              </tbody>
            </table>
          </div>
...
<script>
(function(){
  const selTipo = document.getElementById('tipo_precio_combo');
  const wrapPct = document.getElementById('wrap_pct');
  const wrapFix = document.getElementById('wrap_precio_fijo');
  const chkCombo = document.getElementById('permite_combo');

  function syncPromoForm(){
    if (!selTipo) return;
    const tipo = selTipo.value;
    if (wrapPct) wrapPct.style.display = (tipo === 'porcentaje') ? '' : 'none';
    if (wrapFix) wrapFix.style.display = (tipo === 'precio_fijo') ? '' : 'none';

    if (tipo === 'precio_fijo' && chkCombo) {
      chkCombo.checked = true;
    }
  }

  if (selTipo) {
    selTipo.addEventListener('change', syncPromoForm);
    syncPromoForm();
  }
})();
</script>
