        ...
        try {
            ...
            if ($dup) {
                ...
                if ($snap) {
                    ...
                    /* Producto ya marcado como escaneado por otro identificador */
                    ...
                    /* ============================================
                       4) NO ENCONTRADO EN SNAPSHOT
                       buscar contexto global sencillo
                    ============================================ */
...
<div class="wrap">
    <div class="header">
        <div>
            ...
            <div class="badges">
                <span class="badge"><?= h($auditoria['estatus']) ?></span>
        ...
        <div class="actions">
            ...
            <a href="auditorias_nueva.php" class="btn btn-secondary">Nueva auditoría</a>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <form method="POST" action="" autocomplete="off">
            ...
            <div class="scan-box">
                ...
                <div>
                    <button type="submit" name="registrar_escaneo" value="1" class="btn btn-primary">
                        Registrar escaneo
                    </button>
                </div>
            </div>
        ...
        <div class="notice">
            Esta versión acepta escáner tipo teclado. Puedes usar imei1 o imei2. Si el producto ya fue encontrado, marcará duplicado. Si no pertenece al snapshot, lo registrará como incidencia.
        </div>

        <?php if ($ultimoResultado): ?>
            <div class="result <?= h($ultimoResultado['tipo']) ?>">
                <div class="result-title"><?= h($ultimoResultado['titulo']) ?></div>
                <div><?= h($ultimoResultado['mensaje']) ?></div>
                <div class="meta">
                    <strong>IMEI:</strong> <?= h($ultimoResultado['imei']) ?>
                    <?php if (!empty($ultimoResultado['marca']) || !empty($ultimoResultado['modelo'])): ?>
                        <br>
                        <strong>Equipo:</strong>
                        <?= h(trim(($ultimoResultado['marca'] ?? '') . ' ' . ($ultimoResultado['modelo'] ?? ''))) ?>
                        <?php if (!empty($ultimoResultado['color'])): ?> · <?= h($ultimoResultado['color']) ?><?php endif; ?>
                        <?php if (!empty($ultimoResultado['capacidad'])): ?> · <?= h($ultimoResultado['capacidad']) ?><?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
        <?php endif; ?>
