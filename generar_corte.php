...
/* 1) Días con cobros realmente pendientes (sin corte y no generados) */
$sqlDiasPendientes = "
...
foreach ($diasPendientes as $d) {
    $pendientes[$d['fecha']] = (int)$d['total'];
}

$fechaDefault = !empty($pendientes) ? array_key_first($pendientes) : $fechaHoy;

/* 2) Fecha seleccionada para PREVISUALIZAR (GET) */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fecha_operacion'])) {
    ...
    if (count($cobros) === 0) {
        $msg = "<div class='alert alert-info'>
            ⚠ No hay cobros pendientes para generar corte en la fecha <b>" . htmlspecialchars($fecha_operacion) . "</b>.
        </div>";
    } else {
        // Calcular totales
        $total_efectivo = 0.0;
        ...
        // Insertar corte (id_usuario es INT => "iisdddd")
        ...
        $msg = "<div class='alert alert-success'>
            ✅ Corte generado correctamente (ID: $id_corte) para <b>" . htmlspecialchars($fecha_operacion) . "</b>.
        </div>";

        // Mantener la fecha en la vista
        $fechaSeleccionada = $fecha_operacion;
        // (Opcional) PRG:
        // header("Location: generar_corte.php?fecha_operacion={$fechaSeleccionada}"); exit;
    }
}

/* 4) Cobros pendientes para la fecha seleccionada (previsualización) */
$sqlCobrosPend = "
...
/* UX: deshabilitar botón si no hay cobros */
...
<div class="container mt-4">
  ...
  <!-- Form con 2 acciones: Ver cobros (GET) y Generar corte (POST) -->
  <form class="card p-3 shadow mb-4">
    <label class="form-label">Fecha de operación</label>
    <input
      type="date"
      name="fecha_operacion"
      class="form-control"
      value="<?= h($fechaSeleccionada) ?>"
      max="<?= h($fechaHoy) ?>"
      required
    >
