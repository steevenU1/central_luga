...
/*
  Ajusta aquí si tus nombres reales de rol son distintos.
  Ejemplo: 'RH', 'Recursos Humanos', 'Administrador', etc.
*/
$rolesAdmin = ['Admin', 'RH', 'RecursosHumanos', 'Administrador'];
...
function esAdmin(array $rolesAdmin, string $rol): bool {
...
function puedeAutorizarComoAdmin(mysqli $conn, int $solicitudId): bool {
    $sql = "SELECT id
            FROM vacaciones_solicitudes
            WHERE id = ?
              AND status_jefe = 'Aprobado'
...
$usuarioEsAdmin = esAdmin($rolesAdmin, $rolSesion);
$usuarioEsJefe  = esJefe($rolesJefe, $rolSesion);

if (!$usuarioEsAdmin && !$usuarioEsJefe) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($solicitudId <= 0) {
        ...
        try {
            ...
            elseif ($tipoAccion === 'admin') {
                if (!$usuarioEsAdmin) {
                    throw new Exception('No tienes permisos de Admin/RH para esta acción.');
                }

                if (!puedeAutorizarComoAdmin($conn, $solicitudId)) {
                    throw new Exception('Esta solicitud aún no puede ser autorizada por Admin/RH o ya fue procesada.');
                }
                ...
                $mensaje = $nuevoStatus === 'Aprobado'
                    ? 'La solicitud fue aprobada por Admin/RH.'
                    : 'La solicitud fue rechazada por Admin/RH.';
            }
...
if ($usuarioEsAdmin) {
    ...
    // Jefe ve solo las de su equipo
    $filtroWhere = "u.`$columnaJefe` = ?";
    $params[] = $idUsuarioSesion;
    $types .= "i";
} else {
    // Si no hay columna jefe identificable y no es admin, no ve nada
    $filtroWhere = "1=0";
}
...
$sql = "SELECT
        ...
        ORDER BY
            CASE
                WHEN vs.status_jefe = 'Pendiente' THEN 1
                WHEN vs.status_jefe = 'Aprobado' AND vs.status_admin = 'Pendiente' THEN 2
                ELSE 3
            END,
            vs.creado_en DESC,
            vs.id DESC";
    ...
    <div class="panel">
        <div class="panel-head">
            <div>
                ...
                <div class="sub">
                    Flujo: Jefe inmediato → Admin / RH
                </div>
            </div>

            <div class="rol-chip">
                Rol actual: <?= h($rolSesion) ?>
            </div>
        </div>
                ...
                <?php if ($resSolicitudes->num_rows > 0): ?>
                    <?php while ($row = $resSolicitudes->fetch_assoc()): ?>
                        <?php
                            ...
                            if ($usuarioEsAdmin) {
                                $mostrarAccionesAdmin = ($estadoJefe === 'Aprobado' && $estadoAdmin === 'Pendiente');
                            }
                        ?>
                        <tr>
                            <td><?= $idSolicitud ?></td>
                            ...
                            <td>
                                <div class="acciones">
                                    ...
                                    <?php elseif ($mostrarAccionesAdmin): ?>
                                        <form method="post" onsubmit="return confirm('¿Aprobar definitivamente esta solicitud?');">
                                            <input type="hidden" name="id_solicitud" value="<?= $idSolicitud ?>">
                                            <input type="hidden" name="tipo" value="admin">
                                            <input type="hidden" name="accion" value="aprobar">
                                            <button type="submit" class="btn btn-success">Aprobar admin</button>
                                        </form>

                                        <form method="post" onsubmit="return confirm('¿Rechazar definitivamente esta solicitud?');">
                                            <input type="hidden" name="id_solicitud" value="<?= $idSolicitud ?>">
                                            <input type="hidden" name="tipo" value="admin">
                                            <input type="hidden" name="accion" value="rechazar">
                                            <button type="submit" class="btn btn-danger">Rechazar admin</button>
                                        </form>
                                ...
                                <?php if ($estadoJefe === 'Rechazado'): ?>
                                    <div class="small" style="margin-top:8px;">
                                        La solicitud fue rechazada por jefe y ya no pasa a Admin.
                                    </div>
                                <?php elseif ($estadoJefe === 'Pendiente'): ?>
                                    <div class="small" style="margin-top:8px;">
                                        Esperando autorización del jefe inmediato.
                                    </div>
                                <?php elseif ($estadoJefe === 'Aprobado' && $estadoAdmin === 'Pendiente'): ?>
                                    <div class="small" style="margin-top:8px;">
                                        Pendiente de autorización final por Admin / RH.
                                    </div>
                                <?php endif; ?>
                            </td>
                        </tr>
