...
<div class="modal fade" id="modalPromoPrecioFijo" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-light">
        <h5 class="modal-title">
          <i class="bi bi-stars text-warning me-2"></i>Promoción especial disponible
        </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
      </div>
      <div class="modal-body">
        <div class="alert alert-warning mb-3">
          ...
          El equipo principal seleccionado tiene una promoción especial con la financiera elegida.
        </div>
        <div id="promo_precio_fijo_msg" class="small text-muted">
          ¿Deseas aplicar la promoción?
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-luga btn-luga-ghost" id="btnNoAplicarPromoFijo">No aplicar</button>
        <button type="button" class="btn btn-luga btn-luga-success" id="btnAplicarPromoFijo">
          <i class="bi bi-check2-circle me-1"></i> Aplicar promoción
        </button>
      </div>
    </div>
  </div>
...
$(document).ready(function() {
  ...
  const modalPromoPrecioFijo = new bootstrap.Modal(document.getElementById('modalPromoPrecioFijo'));
  ...
  let promoPrecioFijoDecisionKey = '';
  ...
  function aplicarPromoPrecioFijoUI() {
    if (!promoDesc || !promoDesc.id) return;
    if ((promoDesc.tipo_precio_combo || '') !== 'precio_fijo') return;
    ...
    $('#promo_descuento_modo').val('COMBO_PRECIO_FIJO');
    $('#promo_descuento_tag_origen').val('');
    $('#promo_descuento_porcentaje').val(String(promoDesc.precio_combo_fijo || 0));
    $('#promo_descuento_doble_venta').val('0');
    ...
    $.ajax({
      ...
      error: function(xhr){
        console.warn('No se pudo cargar combos para promo especial:', xhr.responseText || xhr.statusText);
      }
    });
  }

  function evaluarModalPromoPrecioFijo() {
    if (!promoDesc || !promoDesc.id) return;
    if ((promoDesc.tipo_precio_combo || '') !== 'precio_fijo') return;
    if ($('#financiera').val() !== (promoDesc.financiera || $('#financiera').val())) return;

    const key = [$('#equipo1').val() || '', $('#financiera').val() || '', promoDesc.id].join('|');
    if (promoPrecioFijoDecisionKey === key) return;

    const p1 = parseFloat($('#equipo1').find('option:selected').data('precio-lista')) || 0;
    const p2 = parseFloat(promoDesc.precio_combo_fijo || 0);
    const totalPromo = p1 + p2;
    const msg = promoDesc.mensaje_modal && String(promoDesc.mensaje_modal).trim() !== ''
      ? promoDesc.mensaje_modal
      : ('Este equipo está en promoción con la financiera seleccionada. El segundo equipo quedará en $' + p2.toFixed(2) + ' y el total estimado será $' + totalPromo.toFixed(2) + '.');

    $('#promo_precio_fijo_msg').text(msg);
    modalPromoPrecioFijo.show();
  }
  ...
  $('#btnAplicarPromoFijo').on('click', function(){
    if (!promoDesc || !promoDesc.id) return;

    promoPrecioFijoDecisionKey = [$('#equipo1').val() || '', $('#financiera').val() || '', promoDesc.id].join('|');
    aplicarPromoPrecioFijoUI();
    modalPromoPrecioFijo.hide();
  });

  $('#btnNoAplicarPromoFijo').on('click', function(){
    promoPrecioFijoDecisionKey = [$('#equipo1').val() || '', $('#financiera').val() || '', (promoDesc && promoDesc.id) ? promoDesc.id : '0'].join('|');
  ...
  function recalcPrecioVenta() {
    ...
    if (isFinanciamientoCombo()) {
      ...
      if ($opt2.length && $opt2.val()) {
        ...
        const promoModo = ($('#promo_descuento_modo').val() || '');
        const descOn = (descComboOn === true) && ($('#promo_descuento_aplicado').val() === '1') && (promoModo === 'COMBO');
        const descPrecioFijoOn = ($('#promo_descuento_aplicado').val() === '1') && (promoModo === 'COMBO_PRECIO_FIJO');

        if (esRegalo) {
          total += 0;
        } else if (descPrecioFijoOn) {
          const pFijo = parseFloat($('#promo_descuento_porcentaje').val()) || 0;
          total += pFijo;
        } else if (descOn) {
  ...
  $('#equipo1').on('change', function() {
    ...
    evaluarPromoDescuentoPrincipal(idInv);

    refreshEquipoLocks();
    recalcPrecioVenta();
  });

  function evaluarPromoDescuentoPrincipal(idInv) {
    if (!idInv) {
      resetPromoDescuentoUI();
      return;
    }
      ...
      success: function(res) {
        if (res && res.ok && res.aplica && res.promo && res.promo.id) {
          ...
          promoDesc = {
            ...
            tipo_precio_combo: String(res.promo.tipo_precio_combo || 'porcentaje'),
            precio_combo_fijo: parseFloat(res.promo.precio_combo_fijo || 0),
            financiera: String(res.promo.financiera || ''),
            mensaje_modal: String(res.promo.mensaje_modal || '')
          };

          $('#promo_descuento_nombre').text(promoDesc.nombre);
          if (promoDesc.tipo_precio_combo === 'precio_fijo' && promoDesc.precio_combo_fijo > 0) {
            $('#promo_descuento_porc_badge').text('$' + promoDesc.precio_combo_fijo.toFixed(2));
            $('#promo_descuento_hint').text('Esta promo deja el segundo equipo en precio especial con la financiera seleccionada.');
            $('#row_chk_desc_combo').addClass('d-none');
            $('#row_btn_desc_doble').addClass('d-none');
            $('#chk_desc_combo').prop('checked', false);
            descComboOn = false;
          ...
          if (promoDesc.tipo_precio_combo === 'precio_fijo' && $('#financiera').val()) {
            evaluarModalPromoPrecioFijo();
          }
        } else {
          resetPromoDescuentoUI();
        }
  ...
  $('#financiera').on('change', function(){
    const idInv = $('#equipo1').val() || '';
    if (!idInv) return;

    if ($('#promo_descuento_modo').val() === 'COMBO_PRECIO_FIJO') {
      $('#promo_descuento_aplicado').val('0');
      $('#promo_descuento_id').val('0');
      $('#promo_descuento_modo').val('');
      $('#promo_descuento_porcentaje').val('0');
