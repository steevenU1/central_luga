...
<head>
  ...
  <style>
    ...
    .kanban-body{ padding: 12px; overflow:auto; flex:1; }
    .task-card{
      background:#fff;
      border: 1px solid rgba(0,0,0,.08);
      border-radius: 14px;
    ...
    .task-card:hover { transform: translateY(-1px); box-shadow: 0 10px 20px rgba(0,0,0,.06); }
    .task-title{ font-weight:700; font-size:.95rem; margin:0 0 6px; }
    .task-meta{ font-size:.82rem; color:#666; display:flex; flex-wrap:wrap; gap:10px; }
    .pill{
...
<body>
<?php
// Navbar (robusto: incluye el que exista)
$__navCandidates = [
  __DIR__ . '/navbar.php',
...
function renderCard(t){
  ...
  const resp = t.responsable_nombre ? `<span class="pill">👤 ${esc(t.responsable_nombre)}</span>` : `<span class="pill">👤 Sin asignar</span>`;
  ...
  return `
    <div class="task-card" draggable="true" data-id="${esc(t.id)}" data-status="${esc(est)}"
      ...
      ${t.descripcion ? `<div class="text-muted small mt-2">${esc(t.descripcion).slice(0,120)}${t.descripcion.length>120?'…':''}</div>` : ``}
    </div>
  `;
}
...
function attachDnD(){
  ...
  cards.forEach(c => {
    c.addEventListener('dragstart', e => {
      ...
      // ayuda a que no se dispare click
      setTimeout(() => { /* nada */ }, 0);
    });
  ...
  zones.forEach(z => {
    ...
    z.addEventListener('drop', async e => {
      ...
      // si se suelta en el mismo status, no hagas nada
      if (fromStatus && newStatus === fromStatus) return;

      // Si lo movieron a Bloqueado => pedir motivo
...
async function moverTarea(id, estatus){
  ...
  if (!j.ok){
    ...
    // refresca para regresar visualmente
    loadBoard();
    return false;
  }
  return true;
...
qs('btnCancelBloqueoDrag').addEventListener('click', () => {
  // Al cancelar, recargamos para “deshacer” el drop visual
...
qs('btnConfirmBloqueoDrag').addEventListener('click', async () => {
  ...
  const rc = await fetch('api_tarea_comentar.php', { method:'POST', body: fc, credentials:'same-origin' });
...
qs('formNueva').addEventListener('submit', async (e) => {
  ...
  const r = await fetch('api_tablero_guardar.php', { method:'POST', body: form, credentials:'same-origin' });
  const j = await r.json();
  ...
  const modal = bootstrap.Modal.getOrCreateInstance(qs('modalNueva'));
  modal.hide();
