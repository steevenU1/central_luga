...
$isSistemas = ($ROL === 'Admin');  // sistemas
$COSTOS_IDS = [66, 8, 7];          // costos allowlist
$isCostos   = in_array($ID_USUARIO, $COSTOS_IDS, true);

// Debug seguro: solo Admin y con ?debug=1
...
function back($id)
{
    header("Location: portal_proyectos_detalle.php?id=" . $id);
    exit;
}
...
try {
    ...
    $id = (int)($_GET['id'] ?? 0);
    if ($id <= 0) {
        header("Location: portal_proyectos_listado.php");
        exit();
    }
  ...
  ");
    ...
    // ===== Estado de espera (solo visual) =====
    $enEsperaSolicitante = ($estatusActual === 'EN_AUTORIZACION_SOLICITANTE' && !$respSolic);
} catch (Throwable $e) {
    if ($DEBUG) {
...
<body class="bg-light">
    ...
    <div class="container py-4">
        ...
        <?php if ($enEsperaSolicitante): ?>
            <div class="alert alert-info shadow-sm d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    <div class="fw-semibold">⏳ Esperando respuesta del solicitante…</div>
                    <div class="small-muted">
                        Esta pantalla se actualizará automáticamente.
                    </div>
                </div>
                <div class="small-muted">
                    Próxima actualización en <span id="refreshCounter">12</span>s
                </div>
            </div>
        <?php endif; ?>
