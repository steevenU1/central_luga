<?php
// traspaso_nuevo.php — Traspaso ENTRE SUCURSALES (origen = sucursal del usuario)
// Soporta: Equipos por IMEI (estatus→En tránsito) y Accesorios por cantidad (descuenta cantidad)
// UI: Tabs Equipos/Accesorios, carrito lateral, modal de confirmación, acuse en modal (auto-print)
...
$ID_SUC_ORI  = (int)($_SESSION['id_sucursal'] ?? 0);
$NOMBRE_USER = trim($_SESSION['nombre'] ?? 'Usuario');

// ---------- Helpers ----------
function h($s){ return htmlspecialchars($s ?? '', ENT_QUOTES, 'UTF-8'); }
function table_exists(mysqli $c,$t){ $t=$c->real_escape_string($t); $r=$c->query("SHOW TABLES LIKE '{$t}'"); return $r && $r->num_rows>0; }

// ---------- Auto-migración: detalle accesorios para traspasos ----------
$conn->query("
  CREATE TABLE IF NOT EXISTS detalle_traspaso_acc (
    ...
    INDEX (id_producto),
    CONSTRAINT fk_dta_trasp FOREIGN KEY (id_traspaso) REFERENCES traspasos(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
");

// ---------- Estado respuesta/acuse ----------
$mensaje    = '';
$acuseUrl   = '';
$acuseReady = false;
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  $ID_SUC_DEST = (int)($_POST['sucursal_destino'] ?? 0);

  // equipos[]: ids inventario (equipos con IMEI)
  $equiposSeleccionados = (isset($_POST['equipos']) && is_array($_POST['equipos']))
    ? array_values(array_unique(array_map('intval', $_POST['equipos'])))
    : [];

  // acc_qty[ID_INVENTARIO] = cantidad a traspasar (accesorios sin IMEI)
  $accQty = [];
  ...
  if ($ID_SUC_DEST<=0) {
  ...
  } elseif ($ID_SUC_DEST === $ID_SUC_ORI) {
    $mensaje = "<div class='alert alert-warning alert-dismissible fade show shadow-sm'>El destino no puede ser la misma sucursal de origen.<button type='button' class='btn-close' data-bs-dismiss='alert'></button></div>";
    ...
    try {
      // 1) Cabecera traspaso (Pendiente)
      $stCab = $conn->prepare("INSERT INTO traspasos (id_sucursal_origen, id_sucursal_destino, usuario_creo, estatus) VALUES (?,?,?,'Pendiente')");
      $stCab->bind_param("iii", $ID_SUC_ORI, $ID_SUC_DEST, $ID_USUARIO);
      $stCab->execute();
      $ID_TRASPASO = (int)$stCab->insert_id;
      $stCab->close();

      // 2) EQUIPOS (por IMEI) → poner En tránsito + detalle_traspaso
      if (!empty($equiposSeleccionados)) {
        ...
        // Validar que están disponibles en la sucursal ORIGEN
        ...
        $stVal->bind_param('i'.$types, $ID_SUC_ORI, ...$equiposSeleccionados);
        $stVal->execute();
        ...
        if (count($validos)!==count($equiposSeleccionados)) {
          throw new Exception("Algunos equipos ya no están disponibles en la sucursal origen.");
        }
        ...
        foreach ($validos as $idInv) {
          $stDet->bind_param("ii", $ID_TRASPASO, $idInv);  $stDet->execute();
          $stUpd->bind_param("ii", $idInv, $ID_SUC_ORI); $stUpd->execute();
          if ($stUpd->affected_rows !== 1) throw new Exception("No se pudo poner En tránsito el inventario #$idInv");
        }
        $stDet->close(); $stUpd->close();
      }

      // 3) ACCESORIOS (por cantidad) → descontar cantidad en origen + detalle_traspaso_acc
      if (!empty($accQty)) {
        $invIds = array_keys($accQty);
        $place  = implode(',', array_fill(0, count($invIds), '?'));
        $types  = str_repeat('i', count($invIds));

        // Traer renglones válidos del inventario origen
        $sqlAcc = "
        ...
        $stAcc->bind_param('i'.$types, $ID_SUC_ORI, ...$invIds);
        $stAcc->execute();
        ...
        foreach ($accQty as $invId => $qty) {
          if (!isset($map[$invId])) throw new Exception("Accesorio #$invId no disponible en el origen.");
          if ((string)($map[$invId]['imei1'] ?? '') !== '') throw new Exception("El inventario #$invId es equipo con IMEI.");
          $disp = (int)$map[$invId]['cantidad'];
          if ($qty > $disp) throw new Exception("Cantidad superior a disponible (#$invId).");
        }

        // Descontar y registrar detalle
        ...
        foreach ($accQty as $invId => $qty) {
          ...
          $stDetA->bind_param("iiii", $ID_TRASPASO, $invId, $pid, $qty);         $stDetA->execute();
          $stUpdA->bind_param("iiii", $qty, $invId, $ID_SUC_ORI, $qty);          $stUpdA->execute();
      ...
      $conn->commit();
      $acuseUrl   = "acuse_traspaso.php?id={$ID_TRASPASO}&print=1";
      $acuseReady = true;
      $mensaje = "<div class='alert alert-success alert-dismissible fade show shadow-sm'>
        <i class='bi bi-check-circle me-1'></i> <strong>Traspaso #{$ID_TRASPASO}</strong> generado con éxito.
...
$sucursales = [];
$stSuc = $conn->prepare("SELECT id, nombre FROM sucursales WHERE id<>? ORDER BY nombre");
$stSuc->bind_param("i", $ID_SUC_ORI);
$stSuc->execute(); $rs=$stSuc->get_result();
while($r=$rs->fetch_assoc()) $sucursales[]=$r;
$stSuc->close();

// Equipos disponibles (con IMEI) en ORIGEN
$sqlEq = "
...
$stEq->bind_param("i", $ID_SUC_ORI);
$stEq->execute();
$rsEq = $stEq->get_result();
$equipos = $rsEq->fetch_all(MYSQLI_ASSOC);
$stEq->close();

// Accesorios disponibles (por cantidad) en ORIGEN
$sqlAcc = "
...
";
$stAc = $conn->prepare($sqlAcc);
$stAc->bind_param("i", $ID_SUC_ORI);
$stAc->execute();
$rsAc = $stAc->get_result();
$accesorios = $rsAc->fetch_all(MYSQLI_ASSOC);
$stAc->close();
...
<head>
  <meta charset="UTF-8">
  <title>Traspaso entre sucursales</title>
...
<div class="container my-4">
  ...
  <div class="page-header d-flex align-items-center justify-content-between mb-3">
    <div>
      <h1 class="h4 mb-1"><i class="bi bi-arrow-left-right me-2"></i>Traspaso entre sucursales</h1>
      <div class="muted">
        <span class="badge rounded-pill text-bg-light border"><i class="bi bi-house-door me-1"></i>Origen: #<?= (int)$ID_SUC_ORI ?></span>
        <span class="badge rounded-pill text-bg-light border ms-2"><i class="bi bi-person me-1"></i><?= h($NOMBRE_USER) ?></span>
      </div>
    </div>
      ...
      <div class="col-lg-9">
        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade show active" id="tab-equipos" role="tabpanel">
            <div class="card mt-2">
              <div class="card-header d-flex justify-content-between align-items-center">
                <strong>Inventario de equipos (origen)</strong>
                ...
                <div class="table-responsive" style="max-height:420px; overflow:auto;">
                  <table class="table table-hover align-middle mb-0" id="tablaEquipos">
                    ...
                    <?php else: foreach($equipos as $row): ?>
                      <tr data-id="<?= (int)$row['id'] ?>" data-modelo="<?= h($row['modelo']) ?>" data-color="<?= h($row['color']) ?>" data-imei1="<?= h($row['imei1']) ?>">
          ...
          <div class="tab-pane fade" id="tab-accesorios" role="tabpanel">
            <div class="card mt-2">
              <div class="card-header d-flex justify-content-between align-items-center">
                <strong>Inventario de accesorios (origen)</strong>
                ...
                <div class="table-responsive" style="max-height:420px; overflow:auto;">
                  <table class="table table-hover align-middle mb-0" id="tablaAcc">
                    ...
                    <?php else: foreach($accesorios as $row): ?>
                      <tr data-id="<?= (int)$row['id'] ?>" data-modelo="<?= h($row['modelo']) ?>" data-color="<?= h($row['color']) ?>" data-disp="<?= (int)$row['cantidad'] ?>">
                        <td class="fw-semibold"><?= (int)$row['id'] ?></td>
                        <td><?= h($row['marca']) ?></td>
                        <td><?= h($row['modelo']) ?></td>
                        <td><span class="chip"><?= h($row['color']) ?></span></td>
...
// ---- Cantidades accesorios inc/dec + clamp ----
...
document.querySelectorAll('#tablaEquipos .chk-eq').forEach(chk=>{ chk.addEventListener('change', rebuildCart); });
...
document.getElementById('btnRevisar').addEventListener('click', ()=>{
  ...
  fillConfirmModal(); modalConfirm.show();
});
...
document.getElementById('btnConfirmarSubmit').addEventListener('click', ()=>{
  modalConfirm.hide(); document.getElementById('formTraspaso').submit();
});
...
// ---- Modal ACUSE auto (si $acuseReady) ----
...
(function(){
  ...
  modal.show();
  let done=false;
  frame.addEventListener('load', ()=>{ if(done) return; if(frame.src==='about:blank') return; done=true; setBusy(false); });
  const fallback=setTimeout(()=>{ if(!done) setBusy(false); },7000);
  requestAnimationFrame(()=>{ frame.src = ACUSE_URL; });
  modalEl.addEventListener('hidden.bs.modal', ()=>{ clearTimeout(fallback); frame.src='about:blank'; setBusy(false); });
})();
</script>
</body>
</html>
