...
foreach ($data as $r) {
  ...
  $esGer = (strcasecmp($r['rol'], 'Gerente') === 0);

  if (!$esGer) {
    // Mantener regla anterior para equipos/sims/posp (solo ventas NO hechas por gerente)
  ...
  // ✅ TC sí se suma SIEMPRE (incluye las TC vendidas por el gerente)
...
<body>
  ...
  <script>
    ...
    document.getElementById('btn_export').addEventListener('click', () => {
      const COL_BONOS = 14; // <- índice de la columna "Bonos" en tu tabla actual

      const rows = [...document.querySelectorAll('.tbl tr')].map(tr => [...tr.children]
        .filter((_, idx) => idx !== COL_BONOS) // <- NO incluir "Bonos" (input)
        .map(td => {
          const inp = td.querySelector('input');
          return (inp ? inp.value : td.innerText).replace(/\s+/g, ' ').trim();
        })
      );
