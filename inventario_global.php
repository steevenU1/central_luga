...
<head>
  ...
  <style>
    ...
    #tablaInventario td:nth-child(9),
    #tablaInventario th:nth-child(9) {
      max-width: 180px;
    }

    #tablaInventario td:nth-child(9) .truncate {
      display: inline-block;
      max-width: 180px;
      white-space: nowrap;
      overflow: hidden;
...
<body>
          ...
          <div class="row g-3 align-items-end">
            ...
            <div class="col-6 col-md-2">
              ...
              <select name="antiguedad" class="form-select">
                <option value="">Todas</option>
                <option value="<30" <?= $filtroAntiguedad == '<30' ? 'selected' : '' ?>>
                  < 30 días</option>
    ...
    <div class="d-flex justify-content-between mb-3 flex-wrap gap-3">
      <div class="card p-3 shadow-sm" style="max-width:480px; width:100%;">
        ...
        <div class="mt-2 d-flex gap-2">
          <span class="ant-pill lt"><span class="status-dot dot-green me-1"></span>
            < 30 días: <?= (int)$rangos['<30'] ?></span>
  ...
  <div class="modal fade" id="imeiStatusModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body" id="imeiStatusBody">
          <!-- contenido dinámico -->
        </div>
  ...
  <script>
    ...
    // ===== DataTable + Quick Search IMEI =====
    let dt;
    $(function() {
      // Extensión de filtro: OR entre IMEI1 (col 6) e IMEI2 (col 7)
      $.fn.dataTable.ext.search.push(function(settings, data) {
        if (settings.nTable.id !== 'tablaInventario') return true;
        const q = (window.__qImei || '').trim();
        if (q === '') return true;
        const imei1 = (data[6] || '').toString();
        const imei2 = (data[7] || '').toString();
      ...
      // Quick Search IMEI (debounced) + lookup si no hay coincidencias
      const $q = $('#qImei');
      ...
      function applyQuickFilter(val) {
        ...
        // ⚠️ Siempre consultar el backend; si detecta TRASPASO o RETIRO, abre modal,
        // aunque existan filas visibles en la tabla.
        fetch('imei_lookup_status.php?imei=' + encodeURIComponent(q))
          .then(r => r.json())
          .then(payload => {
            if (!payload) return;
            if (payload.status === 'ok') {
              // Solo mostramos si hay al menos traspaso o retiro
              ...
              // Si no hay nada en inventario/traspaso/retiro y tampoco hay coincidencias visibles, avisa.
          ...
          .catch(() => {
            // Silencioso; no bloquear búsqueda
          });
      }
      ...
      // Si venía ?imei=... desde el server-side, sincronizamos al quick search
      if ($q.val().trim() !== '') {
    ...
    function cargarTopVendidos(rango = 'historico') {
      fetch('top_productos.php?rango=' + encodeURIComponent(rango))
        ...
        .catch(() => {
          document.getElementById('tabla-top').innerHTML = '<div class="text-danger small">No se pudo cargar el Top.</div>';
        });
    }
